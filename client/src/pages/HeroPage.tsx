...
import { CalendarIcon, Users, ChevronDownIcon, MapPinIcon, ClockIcon } from 'lucide-react';
...
export function Hero({ startDate, duration, onDateChange, onDurationChange, durations }: {
  ...
  const [videoUrl, setVideoUrl] = useState<string>('/images/heritage-hero.svg');
  const [isGenerating, setIsGenerating] = useState(false);

  useEffect(() => {
    // Generate Marcus Aurelius video/image on component mount
    generateMarcusAureliusVideo();
  }, []);

  const generateMarcusAureliusVideo = async () => {
    try {
      setIsGenerating(true);
      const response = await fetch('/api/generate-marcus-aurelius-video', {
        method: 'POST',
        headers: {
      ...
      console.error('Failed to generate Marcus Aurelius video:', error);
    } finally {
  ...
  return (
    <header className="relative h-[80vh] bg-cover bg-center" style={{ backgroundImage: `url('${videoUrl}')` }}>
      <div className="absolute inset-0 bg-black bg-opacity-50"></div>
      {isGenerating && (
        <div className="absolute top-4 right-4 bg-white bg-opacity-90 px-3 py-2 rounded-lg text-sm">
          Generating Marcus Aurelius era scene...
        </div>
      )}
...
export function Filters({ filters, setFilters, eras, regions }: {
  filters: { era: string; region: string; duration: number };
  setFilters: React.Dispatch<React.SetStateAction<{ era: string; region: string; duration: number }>>;
  eras: string[];
  regions: string[];
}) {
  const toggleFilter = (key: 'era' | 'region', value: string) => {
    setFilters(prev => ({
      ...prev,
